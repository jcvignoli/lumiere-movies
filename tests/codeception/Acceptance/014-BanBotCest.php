...
class BanBotCest {
	...
	public function userAgentShouldBan( AcceptanceTester $I, \Codeception\Example $example ) {
		...
		$url_nonce_valid = $I->CustomGenerateNonce( $I->getCustomBaseUrl() . $example[0], 'popup_nonce' );
		...
		if ( preg_match( '~(' . BAN_BOTS_MSG . ')~', $result, $match ) > 0 ) {
			$I->comment( 'Text "' . $match[0] . '" found' );
	...
	/**
	 * Check if calling popups pages is banned
	 * Popups should ban if no nonce was passed
	 *
	 * @example ["/lumiere/person/?mid=0248281"]
	 * @example ["/lumiere/film/?film=interstellar"]
	 * @example ["/lumiere/movie_search/?film=interstellar"]
	 */
	public function noNonceShouldBan( AcceptanceTester $I, \Codeception\Example $example ) {
		...
		// Try in both languages, fails if both unavailable.
		if ( preg_match( '~(' . BAN_NONCE_MSG . '|' . BAN_NONCE_MSG_FR . ')~', $result ) > 0 ) {
			$I->comment( 'Text found either in English or French' );
			$I->comment( '-> The page correctly banned access to ' . $I->getCustomBaseUrl() . $example[0] );
			$I->comment( $result );
		} else {
			codecept_debug( $result );
	...
	/**
	 * Check if calling popups pages is banned
	 * Popups should ban if an invalid nonce was passed
	 *
	 * @example ["/lumiere/person/?mid=0248281"]
	 * @example ["/lumiere/film/?film=interstellar"]
	 * @example ["/lumiere/movie_search/?film=interstellar"]
	 */
	public function noValidNonceShouldBan( AcceptanceTester $I, \Codeception\Example $example ) {

		$url_nonce_invalid = $I->CustomGenerateNonce( $I->getCustomBaseUrl() . $example[0], 'randomActionThatIsInvalid' );
	...
	public function postsShouldNotBanBots( AcceptanceTester $I, \Codeception\Example $example ) {
		...
		if ( preg_match( '~(' . BAN_BOTS_MSG . '|' . BAN_NONCE_MSG . '|' . BAN_NONCE_MSG_FR . ')~', $result ) === 0 ) {
	...
	private function popupsShouldNotBanLoggedin( AcceptanceTester $I, \Codeception\Example $example ) {
		...
		// Try in both languages, fails if both unavailable.
		if ( preg_match( '~(' . BAN_BOTS_MSG . '|' . BAN_NONCE_MSG . '|' . BAN_NONCE_MSG_FR . ')~', $result ) === 0 ) {
			$I->comment( 'No English or French text found' );
			$I->comment( '-> The popups page correctly did not ban access to ' . $I->getCustomBaseUrl() . $example[0] );
		} else {
			codecept_debug( $result );
			Assert::fail( '!! Banned although logged in, error!' );
		}
	...
	public function userAgentShouldNotBan( AcceptanceTester $I, \Codeception\Example $example ) {
		...
		// Try in both languages, fails if both unavailable.
		if ( preg_match( '~(' . BAN_BOTS_MSG . '|' . BAN_NONCE_MSG . '|' . BAN_NONCE_MSG_FR . ')~', $result ) === 0 ) {
			$I->comment( 'No English or French ban text found' );
