...
2. Should you want to fine-tune your blog, configure the plugin (in admin settings). Default values are automatically filled, no change is needed for a normal use.
...
There are four ways to use Lumière!: 1/ with the popup link maker, 2/ with a widget, 3/ inside a post, 4/ as a list of upcoming movies. Each option can be combined with any other; there is no limitation.
...
4. A block with a list of upcoming movies that can be included in a post.
