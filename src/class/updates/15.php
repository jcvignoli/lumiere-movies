	...
	protected function lumiere_run_local_update(): void {
		...
		foreach ( $posts_array as $post_array ) {
			...
			if ( strlen( $value ) > 0 ) {
				...
				$logger->info( '[updateVersion' . self::LUMIERE_NUMBER_UPDATE . "] Successfully Replaced 'imdb-movie-widget-bymid' by 'lumiere_widget_movieid' in postID $post_id " );
			}
		}
		...
		foreach ( $posts_array as $post_array ) {
			...
			if ( strlen( $value ) > 0 ) {
				...
				$logger->info( '[updateVersion' . self::LUMIERE_NUMBER_UPDATE . "] Successfully Replaced 'imdb-movie-widget' by 'lumiere_widget_movietitle' in postID $post_id " );
			}
		}
		...
		foreach ( $posts_array as $post_array ) {
			...
			$logger->info( '[updateVersion' . self::LUMIERE_NUMBER_UPDATE . "] Successfully deleted 'lumiere_queryid_widget' in postID $post_array->ID " );
		}
		$args = [
			'posts_per_page' => -1,
		...
		foreach ( $posts_array as $post_array ) {
			...
			$logger->info( '[updateVersion' . self::LUMIERE_NUMBER_UPDATE . "] Successfully deleted 'lumiere_queryid_widget_input' in postID $post_array->ID " );
		}
