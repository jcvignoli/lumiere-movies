...
final class Lumiere_Update_File_23 extends \Lumiere\Updates {
	...
	protected function lumiere_run_local_update(): void {
		...
		$this->logger->debug( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . '] Updating post spans and gutenberg tags' );
		 ...
		 */
		$this->logger->debug( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . '] Updating widget, removing [lumiereWidget]' );
		...
		$this->logger->debug( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . '] Updating metada' );
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_metadata( 'post', $post->ID, '_lum_movie_id_widget', $meta_value ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_movieid* in postID $post->ID to *_lum_movie_id_widget*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_movieid' in postID $post->ID metadata: " . strval( $meta_value ) );
			}
		}
		// page
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_post_meta( $post->ID, '_lum_form_type_query', 'lum_movie_id' ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_movieid* in Page postID $post->ID to *lum_movie_id*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_movieid' in Page postID $post->ID" );
			}
		}
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_metadata( 'post', $post->ID, '_lum_movie_title_widget', $meta_value ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_movietitle* in postID $post->ID to *_lum_movie_title_widget*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_movietitle' in postID $post->ID metadata: " . strval( $meta_value ) );
			}
		}
		// page
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_post_meta( $post->ID, '_lum_form_type_query', 'lum_movie_title' ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_movietitle* in Page postID $post->ID to *lum_movie_title*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_movietitle' in Page postID $post->ID" );
			}
		}
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_metadata( 'post', $post->ID, '_lum_person_name_widget', $meta_value ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_personname* in postID $post->ID to *_lum_person_name_widget*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_personname' in postID $post->ID metadata: " . strval( $meta_value ) );
			}
		}
		// page
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_post_meta( $post->ID, '_lum_form_type_query', 'lum_person_name' ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_personname* in Page postID $post->ID to *lum_person_name*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_personname' in Page postID $post->ID" );
			}
		}
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_metadata( 'post', $post->ID, '_lum_person_id_widget', $meta_value ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_personid* in postID $post->ID to *_lum_person_id_widget*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_personid' in postID $post->ID" );
			}
		}
		// page
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value !== false && add_post_meta( $post->ID, '_lum_form_type_query', 'lum_person_id' ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_widget_personid* in Page postID $post->ID to *lum_person_id*" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_widget_personid' in Page postID $post->ID" );
			}
		}
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value === 'enabled' && add_metadata( 'post', $post->ID, '_lum_autotitle_perpost', '0' ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_autotitlewidget_perpost* in postID $post->ID to *_lum_autotitle_perpost*" );
			} elseif ( $meta_value === 'disabled' && add_metadata( 'post', $post->ID, '_lum_autotitle_perpost', '1' ) !== false ) {
				delete_metadata( 'post', $post->ID, 'lumiere_autotitlewidget_perpost' );
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated 'lumiere_autotitlewidget_perpost' in postID $post->ID" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_autotitlewidget_perpost' in postID $post->ID" );
			}
		}
		//page
		...
		foreach ( $posts_array as $post ) {
			...
			if ( $meta_value === 'enabled' && add_post_meta( $post->ID, '_lum_autotitle_perpost', '0' ) !== false ) {
				...
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated *lumiere_autotitlewidget_perpost* in Page postID $post->ID to *_lum_autotitle_perpost*" );
			} elseif ( $meta_value === 'disabled' && add_post_meta( $post->ID, '_lum_autotitle_perpost', '1' ) !== false ) {
				delete_post_meta( $post->ID, 'lumiere_autotitlewidget_perpost' );
				$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully updated 'lumiere_autotitlewidget_perpost' in Page postID $post->ID" );
			} else {
				$this->logger->error( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Error, couldn't update 'lumiere_autotitlewidget_perpost' in Page postID $post->ID" );
			}
		}
		 ...
		 */
		$this->logger->debug( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . '] Updating metada lumiere_autowidget_perpost' );
		...
		foreach ( $posts_array as $post_array ) {
			...
			$this->logger->info( '[updateVersion' . (string) self::LUMIERE_NUMBER_UPDATE . "] Successfully deleted *lumiere_autowidget_perpost* in postID $post_array->ID" );
		}
