 ...
 * @phpstan-extends Parent_Module<'cinematographer', CreditsArrayDef, \Lumiere\Vendor\Imdb\Title>
 * @phan-type CreditsArrayDefPhan = array<int, array{imdb: string, name: string, jobs?: array<int, string>}>
 * @phan-extends Parent_Module<'cinematographer', CreditsArrayDefPhan, \Lumiere\Vendor\Imdb\Title>
 */
final class Movie_Cinematographer extends Parent_Module implements Interface_Movie_Taxonomy {
	...
	public function get_module_taxo( \Lumiere\Vendor\Imdb\Title $movie, string $item_name ): string {
		...
		for ( $i = 0; $i < $nb_total_items; $i++ ) {
			$taxo_options = $this->add_taxo_class->create_taxonomy_options( 'cinematographer', $item_results[ $i ]['name'], $this->settings->get_admin_options() );
