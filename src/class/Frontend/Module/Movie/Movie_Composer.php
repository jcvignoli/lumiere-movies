 ...
 */
final class Movie_Composer extends Parent_Module implements Interface_Movie_Taxonomy {
	...
	public function get_module_taxo( \Lumiere\Vendor\Imdb\Title $movie, string $item_name ): string {
		...
		for ( $i = 0; $i < $nb_total_items; $i++ ) {
			$taxo_options = $this->add_taxo_class->create_taxonomy_options( $item_name, $item_results[ $i ]['name'], $this->settings->get_admin_options() );
