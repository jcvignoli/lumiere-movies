...
final class Taxonomy {
	...
	public function update_custom_terms( string $old_taxonomy, string $new_taxonomy ): void {
		...
		$this->logger?->debug( '[Taxonomy][Start] Replacing taxonomy *' . $old_taxonomy . '* by ' . $new_taxonomy . ' started' );
			...
			if ( ! $terms instanceof \WP_Error ) {
				foreach ( $terms as $term ) {
					...
					if ( $term_deleted === true ) {
						$this->logger?->debug( '[Taxonomy][Update terms][Deleted] Term "' . $term->name . '" deleted from taxonomy "' . sanitize_text_field( $full_old_taxonomy . '"' ) );
					}
				}
			}
			$this->logger?->debug( '[Taxonomy][Update terms][Processed] Term "' . sanitize_text_field( $full_new_taxonomy ) );
		}
		$this->logger?->debug( '[Taxonomy][Update terms][End] taxonomy terms have been processed.' );
	}
	...
	public function create_custom_taxonomy( string $taxonomy = '', string $object_type = '', array $args = [] ): void {
		...
		// $this->logger?->debug( '[Taxonomy] create_custom_taxonomy()' . $taxonomy);
		$get_taxo_array = Get_Options::get_taxonomy_activated(); // Retrieve an array of vars such as "lumiere-director"
	...
	private function update_taxonomy_terms( int $page_id, string $full_new_taxonomy, string $full_old_taxonomy, string $title ): bool {
		...
		$this->logger?->info( '[Taxonomy][Update terms] Regular taxonomy version started' );

		$terms_post = get_the_terms( $page_id, $full_old_taxonomy );
		$this->logger?->debug( '[Taxonomy][Update terms][Post] Title "' . esc_html( $title ) . '" being processed' );
		...
		foreach ( $terms_post as $key => $term_post ) {
			...
			if ( ! $adding_terms instanceof \WP_Error && count( $adding_terms ) > 0 ) {
				$this->logger?->debug( '[Taxonomy][Update terms][Added] Term *' . esc_html( $term_post->name ) . '* to post *' . esc_html( $title ) . '*' );
			}
			$this->logger?->debug( '[Taxonomy][Update terms][Process] Term *' . esc_html( $term_post->name ) );
		}
		$this->logger?->debug( '[Taxonomy][Update terms][Post] Title *' . esc_html( $title ) . '* processed.' );
		return true;
	}
}
