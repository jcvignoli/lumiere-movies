...
class Popup_Person {
	...
	private function find_person( string $nonce_url ): bool {
		...
		$this->mid_sanitized = wp_verify_nonce( $nonce_url ) !== false && $mid_sanitized !== null && strlen( $mid_sanitized ) > 0 ? $mid_sanitized : null;
	...
	public function lumiere_popup_person_layout( string $template_path ): string {
		...
		if ( $this->find_person( $nonce_url ) === false ) {
			status_header( 404 );
		...
		//--------------------------------------------------------------------------- full filmography
		if (
			wp_verify_nonce( $nonce_url ) > 0 && $get_info_person === 'filmo'
		) {
		...
		// ------------------------------------------------------------------------------ partie bio
		if (
			wp_verify_nonce( $nonce_url ) > 0 && $get_info_person === 'bio'
		) {
		...
		// ------------------------------------------------------------------------------ misc part
		if (
			wp_verify_nonce( $nonce_url ) > 0 && $get_info_person === 'misc'
		) {
