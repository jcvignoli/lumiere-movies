...
<div class="lumiere_wrap">
	...
	<div class="lumiere_border_shadow">
		...
		<div class="lumiere_padding_five" id="select_modal_window">&nbsp;
			...
			echo '<div id="bootstrap_explain" class="' . esc_attr( $lumiere_hide_for_bootstrap ) . '">' . wp_kses( wp_sprintf( __( 'Only the width value can be edited with bootstrap modal window. The value entered will be matched against these incremental steps: %1$s300%2$s (small size), %1$s500%2$s (medium size), %1$s800%2$s (large size), %1$s1140%2$s (extra large size)', 'lumiere-movies' ), '<i>', '</i>' ), [ 'i' => [] ] ) . '</div>';
			echo '<div>' . esc_html__( 'Default: Bootstrap', 'lumiere-movies' ) . '</div>';
			echo '</div>';
			?>
		</div>
