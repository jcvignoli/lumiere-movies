	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		function openPopup( url, width, height ) {
			window.open(
				url,
				'popup',
				`resizable=yes, toolbar=no, scrollbars=yes, location=no, width=${ width }, height=${ height }, top=5, left=5`
			);
		}

		/** People popup */
		jQuery( 'a[data-modal_window_people]' ).click( function () {
			const url = `${ lumiere_vars.urlpopup_person }?mid=${ jQuery(
				this
			).data( 'modal_window_people' ) }&_wpnonce=${ jQuery( this ).data(
				'modal_window_nonce'
			) }`;
		...
		jQuery( 'a[data-modal_window_film]' ).click( function () {
			const url = `${ lumiere_vars.urlpopup_film }?film=${ jQuery(
				this
			).data( 'modal_window_film' ) }&_wpnonce=${ jQuery( this ).data(
				'modal_window_nonce'
			) }`;
			openPopup( url, lumiere_vars.popupLarg, lumiere_vars.popupLong );
		} );
		...
		jQuery( 'a[data-modal_window_filmid]' ).click( function () {
			const url = `${ lumiere_vars.urlpopup_film }?mid=${ jQuery(
				this
			).data( 'modal_window_filmid' ) }&_wpnonce=${ jQuery( this ).data(
				'modal_window_nonce'
			) }`;
			openPopup( url, lumiere_vars.popupLarg, lumiere_vars.popupLong );
		} );
