( function () {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		if (
			jQuery( 'input.activatehidesectionAdd' ).prop( 'checked' ) == true
		) {
			jQuery( '.hidesectionOfCheckbox' ).show();
		} else if (
			jQuery( '.activatehidesectionRemove' ).prop( 'checked' ) == true
		) {
			jQuery( '.hidesectionOfCheckbox' ).hide();
		}

		jQuery( 'input.activatehidesectionAdd' )
			.click( function () {
				jQuery( '.hidesectionOfCheckbox' ).slideToggle();
			} )
			.nextAll( '.hidesectionOfCheckbox' )
			.show();

		if (
			jQuery( 'input.activatehidesectionAddTwo' ).prop( 'checked' ) ==
			true
		) {
			jQuery( '.hidesectionOfCheckboxTwo' ).show();
		} else if (
			jQuery( '.activatehidesectionRemoveTwo' ).prop( 'checked' ) == true
		) {
			jQuery( '.hidesectionOfCheckboxTwo' ).hide();
		}

		jQuery( 'input.activatehidesectionAddTwo' )
			.click( function () {
				jQuery( '.hidesectionOfCheckboxTwo' ).slideToggle();
			} )
			.nextAll( '.hidesectionOfCheckboxTwo' )
			.show();
		 ...
		 */
		jQuery( '[data-show-hidden]' ).click( function () {
			const htmltag_id_to_change = jQuery( this ).data( 'show-hidden' );
