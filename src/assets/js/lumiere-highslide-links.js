( function ( jQuery ) {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		jQuery( '.lum_pic_inpopup' ).click( function () {
			return hs.expand( this, {
				useBox: false,
				captionEval: 'Poster of ' + 'this.thumb.alt',
			} );
		} );

		/* Class for a HTML tag a, available into the post only */
		jQuery( '.lum_pic_link_highslide' ).click( function () {
			return hs.expand( this, {
				useBox: false,
				captionEval: 'Poster of ' + 'this.thumb.alt',
			} );
		...
		jQuery( '#highslide_button_close' ).click( function ( e ) {
			console.log( 'test' );
			hs.close( e );
		} );
		...
		jQuery( 'a[data-modal_window_people]' ).click( function () {
			...
			const modal_window_people = jQuery( this )
				.closest( 'a' )
				.data( 'modal_window_people' );
			const modal_window_nonce = jQuery( this )
				.closest( 'a' )
				.data( 'modal_window_nonce' );

			const url_imdbperso = `${ lumiere_vars.urlpopup_person }?mid=${ modal_window_people }&_wpnonce=${ modal_window_nonce }`;
		...
		jQuery( 'a[data-modal_window_filmid]' ).click( function () {
			...
			const tmppopupLarg = lumiere_vars.popupLarg;
			const tmppopupLong = lumiere_vars.popupLong;
			const modal_window_filmid = jQuery( this )
				.closest( 'a' )
			...
			const url_imdbperso = `${ lumiere_vars.urlpopup_film }?mid=${ modal_window_filmid }&_wpnonce=${ modal_window_nonce }`;
