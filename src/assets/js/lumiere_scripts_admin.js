( function () {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		jQuery( 'input[data-modificator]' ).change( function () {
			if ( jQuery( this ).is( ':checked' ) ) {
				const optionOne = jQuery( this )
					.closest( 'input' )
					.data( 'field_to_change' );
				const optionTwo = jQuery( this )
					.closest( 'input' )
					.data( 'field_to_change_value' );
		...
		jQuery( 'input[data-modificator2]' ).change( function () {
			if ( jQuery( this ).is( ':checked' ) ) {
				const optionOne = jQuery( this )
					.closest( 'input' )
					.data( 'field_to_change2' );
				const optionTwo = jQuery( this )
					.closest( 'input' )
					.data( 'field_to_change_value2' );
				GereControle( optionOne, optionTwo );
			}
		} );
		...
		jQuery( 'input[data-modificator3]' ).change( function () {
			if ( jQuery( this ).is( ':checked' ) ) {
				const optionOne = jQuery( this )
					.closest( 'input' )
					.data( 'field_to_change3' );
				const optionTwo = jQuery( this )
					.closest( 'input' )
					.data( 'field_to_change_value3' );
				GereControle( optionOne, optionTwo );
			}
		} );
		...
		jQuery( 'input[data-checkbox_activate]' ).change( function () {
			const htmltag_id_to_change = jQuery( this )
				.closest( 'input' )
				.data( 'checkbox_activate' );
			jQuery( '#' + htmltag_id_to_change ).toggle(
				jQuery( this ).closest( 'input' ).is( ':checked' )
			);
		} );
		...
		jQuery( 'input[data-checkbox_deactivate]' ).change( function () {
			const htmltag_id_to_change = jQuery( this )
				.closest( 'input' )
				.data( 'checkbox_deactivate' );
			jQuery( '#' + htmltag_id_to_change ).toggle(
				jQuery( this ).closest( 'input' ).is( ':not(:checked)' )
			);
		} );
		...
		jQuery( '[data-field_activate]' ).click( function () {
			const htmltag_id_to_change =
				jQuery( this ).data( 'field_activate' );
	...
	function GereControle( Controle, Masquer ) {
		const objControle = document.getElementById( Controle );
		if ( Masquer == '1' ) {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		function moveOptionUp( containerId ) {
			const $opt = jQuery( '#' + containerId + ' option:selected' );
			if ( $opt.is( ':first-child' ) ) {
				$opt.insertAfter(
					jQuery( '#' + containerId + ' option:last-child' )
				);
			} else {
		...
		function moveOptionDown( containerId ) {
			...
			if ( $opt.is( ':last-child' ) ) {
				$opt.insertBefore(
					jQuery( '#' + containerId + ' option:first-child' )
				);
			} else {
		...
		jQuery( '#movemovieup' ).click( function () {
			const containerId = jQuery( this ).data( 'container-id' );
		...
		jQuery( '#imdbconfig_save' ).submit( function () {
			const containerId = jQuery( '#movemovieup' ).data( 'container-id' ); // Assuming the same container ID
	...
	function checkAll( field ) {
		for ( let i = 0, fieldLenght = field.length; i < fieldLenght; i++ ) {
	...
	( function ( $ ) {
		$( document ).on( 'click', 'input[data-check-movies]', function ( e ) {
			checkAll(
				document.getElementsByName( 'imdb_cachedeletefor_movies[]' )
			);
		} );
	} )( jQuery );
	/* people */
	( function ( $ ) {
		$( document ).on( 'click', 'input[data-check-people]', function ( e ) {
			checkAll(
				document.getElementsByName( 'imdb_cachedeletefor_people[]' )
			);
		} );
	} )( jQuery );
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		jQuery( 'input[data-valuemodificator]' ).click( function () {
			const field_option = jQuery( this )
				.closest( 'input' )
				.data( 'valuemodificator_field' );

			if ( jQuery( this ).is( ':checked' ) ) {
				document.getElementById( field_option ).value =
					jQuery( this ).val();
			} else {
				document.getElementById( field_option ).value = jQuery( this )
					.closest( 'input' )
					.data( 'valuemodificator_default' );
			}
		} );
		 ...
		 */
		jQuery( 'input[data-valuemodificator_advanced]' ).click( function () {
			const field_option = jQuery( this )
				.closest( 'input' )
				.data( 'valuemodificator_field' );
			...
			if ( jQuery( this ).is( ':checked' ) ) {
				document.getElementById( field_option ).value = jQuery( this )
					.closest( 'input' )
					.data( 'valuemodificator_valuecurrent' );
			} else {
				document.getElementById( field_option ).value = jQuery( this )
					.closest( 'input' )
					.data( 'valuemodificator_valuedefault' );
			}
		} );
	} );
	 ...
	 */
	document.addEventListener( 'DOMContentLoaded', function () {
		jQuery( 'a[data-lumiere_admin_search_popup]' ).click( function () {
			// Vars from Settings class, transmitted in script lumiere_scripts_admin_vars from class Admin
			const tmppopupLarg = lumiere_admin_vars.popupLarg;
			const tmppopupLong = lumiere_admin_vars.popupLong;
			const url_imdbperso =
			...
			window.open(
				url_imdbperso,
				'popup',
				'resizable=yes, toolbar=no, scrollbars=yes, location=no, width=' +
					tmppopupLarg +
					', height=' +
					tmppopupLong +
					', top=100, left=100'
