...
( function () {
	...
	tinymce.create( 'tinymce.plugins.lumiere_link_maker', {
			...
			return {
				...
				onclick() {
					...
					ed.windowManager.open( {
						...
						onsubmit( e ) {
							ed.insertContent(
								`<span data-lum_link_maker="popup">${ e.data.textboxName }</span>`
							);
						},
					} );
				},
			...
			return {
				...
				onclick() {
					...
					ed.windowManager.open( {
						...
						onsubmit( e ) {
							ed.insertContent(
								`<span data-lum_movie_maker="${ e.data.movieFormat }">${ e.data.movieReference }</span>`
							);
						},
					} );
				},
		 ...
		 */
		removeTemporaryIcons( event ) {
			if ( event.get ) {
				event.content = event.content.replace(
					/<img class="lumiere_admin_tiny_img"[^>]+>\s?\s?/g,
					''
				);
			}
		},
		...
		addTemporaryIcons( event ) {
			...
			event.content = event.content
				.replace(
					/<span data-lum_link_maker="popup">/g,
					icons.span_popup
				)
				.replace(
					/<span data-lum_movie_maker="lum_movie_title">/g,
					icons.span_movie_title
				)
				.replace(
					/<span data-lum_movie_maker="lum_movie_id">/g,
					icons.span_movie_id
				)
				.replace(
					/<span data-lum_movie_maker="lum_person_name">/g,
					icons.span_person_name
				)
				.replace(
					/<span data-lum_movie_maker="lum_person_id">/g,
					icons.span_person_id
				);
		},
