...
const ButtonOpenSearch = ( useBlockProps ) => {
	const handleClick = () => {
		try {
		        const selectedText = useBlockProps.contentRef.current.textContent;
		        open( `${lumiere_admin_vars.wordpress_path}${lumiere_admin_vars.admin_movie_search_url}?select_search_type=movie&${lumiere_admin_vars.admin_movie_search_qstring}=${selectedText}`, 'Lumiere', 'width=800,height=600' );
		} catch (error) {
...
const ConditionalButton = compose(
	...
	ifCondition(
		( useBlockProps ) => (
			useBlockProps.selectedBlock &&
			useBlockProps.selectedBlock.name === 'lumiere/main'
		)
	)
)( ButtonOpenSearch );


registerBlock( jsonData.name,
	{
