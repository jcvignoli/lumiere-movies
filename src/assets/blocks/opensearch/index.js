...
const ConditionalButton = compose(
	...
	ifCondition(
		( { selectedBlock } ) =>
			...
			selectedBlock.attributes.lumiere_imdblt_select.substring(
				selectedBlock.attributes.lumiere_imdblt_select.lastIndexOf(
					'_'
				) + 1
			) != 'id'
	)
...
registerFormatType( jsonData.name, {
	...
	description: __(
		'Open a new window to find the IMDb id for selected movie or person',
		'lumiere-movies'
	),
