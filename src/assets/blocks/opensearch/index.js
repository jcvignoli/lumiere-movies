...
const ButtonOpenSearch = ( { selectedBlock, isActive } ) => {
	...
	const handleClick = () => {
	        open( lumiere_admin_vars.wordpress_path + lumiere_admin_vars.admin_movie_search_url + '?select_search_type=' + finalSearchType + '&' + lumiere_admin_vars.admin_movie_search_qstring + '=' + selectedText, 'Lumiere', 'width=800,height=600' );
	};
...
const ConditionalButton = compose(
	withSelect(
		( select ) => ( { selectedBlock: select( 'core/block-editor' ).getSelectedBlock() } )
	),
	ifCondition(
		( { selectedBlock } ) => (
			selectedBlock &&
			// Same name as main block name in /post
			selectedBlock.name === jsonDataMain.name
			// Don't display if it's an ID selection (will only select title and name)
			&& selectedBlock.attributes['lumiere_imdblt_select'].substring( selectedBlock.attributes['lumiere_imdblt_select'].lastIndexOf("_") + 1 ) != 'id'
		)
	)
)( ButtonOpenSearch );
...
registerFormatType( jsonData.name, {
	...
	description: __( 'Open a new window to find the IMDb id for selected movie or person', 'lumiere-movies' ),
