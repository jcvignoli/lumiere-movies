...
if ((isset($imdb_admin_values['imdbdebug'])) && ($imdb_admin_values['imdbdebug'] == "1")){
	...
	$utilsClass->lumiere_activate_debug($imdb_admin_values, NULL, NULL);
}
...
if ((isset($_GET['msg'])) && array_key_exists( sanitize_key( $_GET['msg'] ), $messages ) ){
	switch (sanitize_text_field( $_GET['msg'] )) {
		...
		case "highslide_success":
			echo $utilsClass->lumiere_notice(1, esc_html__( $messages["highslide_success"], 'lumiere-movies') );
			break;
		// Message for failure
		case "highslide_failure":
			echo $utilsClass->lumiere_notice(3, esc_html__( $messages["highslide_failure"] , 'lumiere-movies') . " " .  esc_html__( 'Your folder might be protected. Download highslide manually', 'lumiere-movies')." <a href='". esc_url ( \Lumiere\Settings::IMDBBLOGHIGHSLIDE ) ."'>".esc_html__("here", 'lumiere-movies')."</a> ".esc_html__("and extract the zip into" ) . "<br />" .  esc_url( $imdb_admin_values['imdbpluginpath'] ."js/" ) );
			break;
		// Message for website down
		case "highslide_down":
			echo $utilsClass->lumiere_notice(3, esc_html__( $messages["highslide_down"] , 'lumiere-movies')  );
			break;
		// Message for website unkown
		case "highslide_website_unkown":
			echo $utilsClass->lumiere_notice(3, esc_html__( $messages["highslide_website_unkown"] , 'lumiere-movies')  );
			break;
	}
}
...
{
			echo $utilsClass->lumiere_notice(3, esc_html__( 'Wrong values. You can not select the same URL string for taxonomy pages and popups.', 'lumiere-movies') );
		...
		// update options
		update_option($configClass->imdbAdminOptionsName, $imdb_admin_values);
		...
		/*if ( lumiere_make_htaccess() ) {
			echo $utilsClass->lumiere_notice(1, esc_html__( 'htaccess file successfully generated.', 'lumiere-movies') );
		} else {
			echo $utilsClass->lumiere_notice(3, esc_html__( 'Can not write htaccess file, check permissions.', 'lumiere-movies') );
		}*/
		...
		delete_option($configClass->imdbAdminOptionsName);
...
<div id="tabswrap">
	<div class="imdblt_double_container lumiere_padding_five">
		<div class="lumiere_flex_auto lumiere_align_center"><img src="<?php echo esc_url( $configClass->lumiere_pics_dir . 'menu/admin-general-path.png'); ?>" align="absmiddle" width="16px" />&nbsp;&nbsp;<a title="<?php esc_html_e( "Paths & Layout", 'lumiere-movies');?>" href="<?php echo esc_url(admin_url() . "admin.php?page=lumiere_options&generaloption=base" ); ?>"><?php esc_html_e( 'Layout', 'lumiere-movies'); ?></a></div>

		<div class="lumiere_flex_auto lumiere_align_center">&nbsp;&nbsp;<img src="<?php echo esc_url( $configClass->lumiere_pics_dir . 'menu/admin-general-advanced.png'); ?>" align="absmiddle" width="16px" />&nbsp;&nbsp;<a title="<?php esc_html_e( "Advanced", 'lumiere-movies');?>" href="<?php echo esc_url (admin_url() . "admin.php?page=lumiere_options&generaloption=advanced" ); ?>"><?php esc_html_e( "Advanced", 'lumiere-movies'); ?></a></div>
	</div>
</div>
...
;
				...
				// No "highslide" folder is found
					...
					// Say so!
					echo $utilsClass->lumiere_notice(4, '<span class="imdblt_red_bold">'.esc_html__('Warning! No Highslide folder was found.', 'lumiere-movies') .'</span>');
					echo "<br />";
					...
					echo '<a href="http://highslide.com/" title="' . esc_html__('Click here to visit Highslide website', 'lumiere-movies') .'"><img src="'.esc_url( $configClass->lumiere_pics_dir . 'menu/admin-general-install-highslide.png') . '" align="absmiddle" />&nbsp;&nbsp;'.esc_html__('Get Highslide JS library', 'lumiere-movies') . '</a><br /><br />';
				}
	...
	<div class="inside imblt_border_shadow">
		...
		<div class="lumiere_flex_container">
			...
			<div id="imdbwordpress_bigmenu" class="lumiere_flex_auto lumiere_padding_five">
				...
				<input type="hidden" id="imdb_imdbwordpress_bigmenu_no" name="imdb_imdbwordpress_bigmenu" value="0" <?php if ($imdb_admin_values['imdbwordpress_bigmenu'] == "1") { echo 'checked="checked"'; }?> />
			...
			<div id="imdbwordpress_tooladminmenu" class="lumiere_flex_auto lumiere_padding_five">
				...
				<input type="hidden" id="imdb_imdbwordpress_tooladminmenu_no" name="imdb_imdbwordpress_tooladminmenu" value="0" <?php if ($imdb_admin_values['imdbwordpress_bigmenu'] == "1") { echo 'checked="checked"'; }?> />
			...
			<div class="lumiere_flex_auto lumiere_padding_five hidesectionOfCheckbox">
				...
				<div class="lumiere_padding_top_bottom_ten">
					...
					<select name="imdb_imdbdebuglevel">
						...
						<option <?php if($imdb_admin_values['imdbdebuglevel'] == "INFO") echo 'selected="selected"'; ?>value="INFO">Info</option>
						<option <?php if($imdb_admin_values['imdbdebuglevel'] == "NOTICE") echo 'selected="selected"'; ?>value="NOTICE">Notice</option>
						<option <?php if($imdb_admin_values['imdbdebuglevel'] == "WARNING") echo 'selected="selected"'; ?>value="WARNING">Warning</option>
						<option <?php if($imdb_admin_values['imdbdebuglevel'] == "ERROR") echo 'selected="selected"'; ?>value="ERROR">Error</option>
						<option <?php if($imdb_admin_values['imdbdebuglevel'] == "CRITICAL") echo 'selected="selected"'; ?>value="CRITICAL">Critical</option>
